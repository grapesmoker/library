...
var LibraryView = Backbone.View.extend({
    initialize: function (options) {
        ...
        console.log(this.collection.length, 'documents')
    },
    ...
    editLibrary: function () {
        ...
        var template = LibraryEditTemplate({
            name: lib_name,
    ...
    showDocuments: function () {
        ...
        console.log('limit:', that.options.limit, 'offset:', that.options.offset)
        documents.fetch({
            data: {limit: that.options.limit, offset: that.options.offset},
            success: function (collection, response, options) {
                collection.set(response['objects'])
                var totalRecords = response['meta']['total_count']
                that.options.totalRecords = totalRecords
    ...
    showNextDocsPage: function () {
        console.log('moving to offset', this.options.offset + this.options.limit)
        if (this.options.offset + this.options.limit * (this.options.currentPage) <= this.options.totalRecords) {
    ...
    showPrevDocsPage: function () {
        console.log('moving to offset', this.options.offset - this.options.limit)
        if (this.options.offset - this.options.limit >= 0) {
            this.options.offset = this.options.offset - this.options.limit
            this.options.currentPage -= 1
            this.showDocuments()
        }
